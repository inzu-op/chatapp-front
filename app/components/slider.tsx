...
export const Sidebar: FC<SidebarProps> = ({
  ...
  const fetchChatUsers = async () => {
    try {
      const response = await fetch(`http://localhost:5000/api/users/chat-users?userId=${currentUserId}`);
      if (response.ok) {
  ...
  const searchUsers = async (query: string) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`http://localhost:5000/api/users/search?query=${encodeURIComponent(query.trim())}`);
      if (response.ok) {
        const data = await response.json();
        setUsers(data);
      } else {
  ...
  const addUserToChat = async (targetUserId: string) => {
    ...
    try {
      const response = await fetch("http://localhost:5000/api/users/add-chat", {
        method: "POST",
        headers: {
  ...
  const removeUserFromChat = async (targetUserId: string) => {
    ...
    try {
      const response = await fetch("http://localhost:5000/api/users/remove-chat", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
